...
public class AttendanceController extends HttpServlet {
    ...
    private void showAttendance(HttpServletRequest req, HttpServletResponse resp, String error, String success)
        ...
        List<ShiftRow> rows = Collections.emptyList();
        String viewError = error;

        if (dao.getConnection() == null) {
            viewError = "Không thể kết nối database. Vui lòng kiểm tra MySQL và DBContext.";
        } else {
            rows = dao.listByDate(sqlDate);
        }
        ...
        if (viewError != null) {
            req.setAttribute("error", viewError);
        }
        if (success != null) {
    ...
    private void handle(HttpServletRequest req, HttpServletResponse resp, String action)
        ...
        if (dao.getConnection() == null) {
            showAttendance(req, resp, "Không thể kết nối database. Vui lòng kiểm tra MySQL và DBContext.", null);
            return;
        }
