    ...
    <body>
        ...
        <div class="wrapper-fullwidth">
            ...
            <div class="main-layout">
                ...
                <div class="sidebar">
                    ...
                    <div class="mb-4">
                        <input type="text" id="keywordSearch" class="search-box-custom"
                               placeholder="Nhập quy mô bàn cần lọc..."
                               onkeyup="filterTablesByKeyword()">
                    </div>

                    <form id="checkForm" method="get" action="${pageContext.request.contextPath}/reservation"
                        ...
                        <div class="mb-3">
                            ...
                            <input type="datetime-local"
                                   ...
                                   name="orderTime" id="orderTime" value="${orderTime}" required>
                        </div>
                                ...
                                <c:choose>
                                    ...
                                    <c:otherwise>
                                        <div class="area-btn ${areaType == 'private' ? 'selected' : ''}"
                                             onclick="selectArea(this, 'private')">🚪 Trong phòng</div>
                                        <div class="area-btn ${areaType == 'public' ? 'selected' : ''}"
                                             onclick="selectArea(this, 'public')">🌿 Ngoài sảnh</div>
                                    </c:otherwise>
                                </c:choose>
                            </div>
                ...
                <div class="main-content">
                    ...
                    <c:if test="${step == 'success'}">
                        <div class="text-center py-4">
                            ...
                            <p class="text-muted small">Mã đơn hàng: <strong>#${order.orderID}</strong></p>
                            ...
                            <div class="card shadow-sm my-4 success-card">
                                <div class="card-body p-4" style="font-size:0.95rem; text-align:left;">
                                    ...
                                    <div class="mb-2">
                                        ...
                                        <c:choose>
                                            <c:when test="${order.areaType == 'public'}">🌿 Ngoài sảnh</c:when>
                                            <c:otherwise>🚪 Trong phòng</c:otherwise>
                                        </c:choose>
                                    </div>
                                    <div class="mb-2">
                                    ...
                                    <div class="mb-2">
                                        ...
                                        <span class="badge badge-reserved rounded-pill">✅ Đã giữ bàn</span>
                                    </div>
                                    <hr style="border-color:#eedec4;">
                                    <p class="text-muted mb-0" style="font-size:0.85rem;">
                                        ℹ️ Bàn đã được giữ cho quý khách. Vui lòng đến đúng giờ và
                                        cho nhân viên biết mã đơn <strong>#${order.orderID}</strong>.
                                        Bàn sẽ tự động hủy nếu quý khách không đến sau <strong>30 phút</strong> kể từ giờ hẹn.
                                    </p>
                                </div>
                            </div>
                            ...
                            <div class="d-flex gap-2 justify-content-center">
                                <a href="${pageContext.request.contextPath}/reservation?action=history"
                                   class="btn btn-outline-secondary rounded-pill px-4 btn-sm">📅 Xem lịch sử</a>
                                <a href="${pageContext.request.contextPath}/"
                                   class="btn btn-dark rounded-pill px-4 btn-sm"
                                   style="background-color:#3d2514; border:none;">🏠 Trang chủ</a>
                            </div>
                        </div>
                    </c:if>
                                            ...
                                            <div class="card border-0 shadow-sm" style="background-color:#fffdf9;">
                                                <div class="card-body p-3">
                                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                                        <span class="fw-bold text-secondary small">Đơn #${o.orderID}</span>
                                                        <c:choose>
                                                            <c:when test="${o.orderStatus == 'reserved'}">
                                                                <span class="badge badge-reserved rounded-pill">✅ Đã giữ bàn</span>
                                                            </c:when>
                                                            <c:when test="${o.orderStatus == 'pending'}">
                                                                <span class="badge badge-pending rounded-pill">⏳ Chờ xác nhận</span>
                                                            </c:when>
                                                            <c:when test="${o.orderStatus == 'cancelled'}">
                                                                <span class="badge badge-cancelled rounded-pill">❌ Đã hủy</span>
                                                            </c:when>
                                                            <c:when test="${o.orderStatus == 'serving'}">
                                                                <span class="badge badge-serving rounded-pill">🍽 Đang phục vụ</span>
                                                            </c:when>
                                                            <c:otherwise>
                                                                <span class="badge badge-completed rounded-pill">${o.orderStatus}</span>
                                                            </c:otherwise>
                                                        </c:choose>
                                                    </div>
                                                    <p class="m-0 small">🪑 Loại bàn: <strong>${o.capacity} chỗ</strong></p>
                                                    <p class="m-0 small">
                                                        📍 Khu vực: <strong>
                                                            <c:choose>
                                                                <c:when test="${o.areaType == 'public'}">Ngoài sảnh</c:when>
                                                                <c:otherwise>Trong phòng</c:otherwise>
                                                            </c:choose>
                                                        </strong>
                                                    </p>
                    ...
                    <c:if test="${step != 'success' && step != 'history'}">
                        <form id="mainBookingForm" method="post"
                            ...
                            <c:if test="${step == 'choose-table'}">
                                <div class="mb-4">
                                    <div class="section-title">3. Chọn quy mô bàn phù hợp</div>
                                                ...
                                                <c:choose>
                                                    ...
                                                    <c:otherwise>
                                                        <div class="table-card ${capacity == g.capacity ? 'selected' : ''}"
                                                            ...
                                                            <span class="badge bg-success rounded-pill fw-normal" style="font-size:.7rem;">
                                ...
                                <div class="mb-4">
                                    ...
                                    <div class="menu-placeholder">
                                        ...
                                        <p class="text-muted mb-0" style="font-size:.8rem;">
                                            Module chọn món đang được cập nhật. Quý khách có thể bấm xác nhận phía dưới để hoàn tất đặt chỗ.
                                        </p>
                                    </div>
                                </div>
                            ...
                            <c:if test="${step == 'pick-time'}">
                                <div class="text-center py-5 text-muted">
                                    ...
                                    <p style="font-size:0.95rem; line-height:1.6;">
                                        ...
                                        sau đó bấm <strong>"Kiểm tra trống"</strong> để hiển thị danh sách bàn khả dụng.
                                    </p>
                                </div>
                            </c:if>
                        </form>
        ...
        <script>
            function filterTablesByKeyword() {
                var filter = document.getElementById("keywordSearch").value.toLowerCase();
            ...
            function selectCapacity(card, capacityValue) {
                ...
                document.getElementById('finalCapacity').value = capacityValue;
                document.getElementById('tableError').style.display = 'none';
            }

            function validateFinalForm() {
                var cap = document.getElementById('finalCapacity').value;
                if (!cap || cap === "" || cap === "-1") {
