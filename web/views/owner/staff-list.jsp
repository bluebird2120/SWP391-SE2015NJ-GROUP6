...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .page-title { margin:0; color:#76493b; font:700 1.6rem 'Playfair Display',serif; }
        .page-sub { color:#a0714f; font-size:.9rem; margin-top:4px; }
        .btn { border:0; border-radius:8px; padding:9px 16px; display:inline-flex; align-items:center; gap:6px; cursor:pointer; text-decoration:none; font-weight:600; font-size:.88rem; }
        ...
        .filter-bar { display:flex; align-items:end; gap:12px; flex-wrap:wrap; background:#fff; border:1px solid #ede0d8; border-radius:12px; padding:16px; margin-bottom:16px; }
        .field { display:flex; flex-direction:column; gap:4px; }
        .field label { color:#8a6e5a; font-size:.78rem; font-weight:600; text-transform:uppercase; }
        .field input,.field select { min-width:200px; padding:8px 12px; border:1px solid #d7bfa4; border-radius:7px; font:inherit; }
        .table-card { background:#fff; border:1px solid #ede0d8; border-radius:12px; overflow:auto; }
        table { width:100%; border-collapse:collapse; min-width:980px; }
        th { background:#faf6f2; padding:12px; text-align:left; color:#76493b; font-size:.8rem; text-transform:uppercase; border-bottom:1px solid #ede0d8; }
        td { padding:12px; border-bottom:1px solid #f5ece4; font-size:.9rem; vertical-align:middle; }
        tr:hover { background:#faf6f2; }
        .avatar { width:36px; height:36px; border-radius:50%; object-fit:cover; background:#ede0d8; }
        .avatar-fallback { display:inline-flex; align-items:center; justify-content:center; color:#76493b; }
        .badge { padding:4px 10px; border-radius:12px; font-size:.75rem; font-weight:600; background:#f1e8df; }
        .badge-active { background:#d4edda; color:#155724; }
        .badge-inactive { background:#f8d7da; color:#721c24; }
        .badge-reception { background:#e8e1ff; color:#5944a8; }
        .actions-cell { display:flex; gap:6px; flex-wrap:wrap; }
        .inline-form { display:inline; }
        .empty { text-align:center; padding:40px; color:#a0714f; }
        .alert { padding:11px 14px; border-radius:8px; margin-bottom:14px; background:#d4edda; color:#155724; }
        .pagination { display:flex; gap:6px; padding:14px 16px; align-items:center; flex-wrap:wrap; }
        .pagination a,.pagination span { padding:6px 12px; border:1px solid #ede0d8; border-radius:6px; color:#76493b; text-decoration:none; font-size:.85rem; }
        .pagination .active { background:#76493b; color:#fff; }
        .pagination .info { margin-left:auto; border:0; color:#8a6e5a; }
        .confirm-modal { display:none; position:fixed; inset:0; z-index:100000; background:rgba(0,0,0,.4); align-items:center; justify-content:center; }
        .confirm-modal.show { display:flex; }
        .confirm-box { width:min(400px,90vw); background:#fff; border-radius:12px; padding:24px; text-align:center; box-shadow:0 8px 32px rgba(0,0,0,.15); }
    ...
    <div style="display:flex;">
        ...
        <main class="main">
            ...
            <form method="get" action="${pageContext.request.contextPath}/owner/staff" class="filter-bar">
                ...
                <%-- [LOC THEO ROLE] Giu chuc nang cua nhanh hien tai khi gop main. --%>
                <div class="field">
                    <label>Role</label>
                    <select name="role">
                            ...
                            <c:otherwise>
                                <c:forEach var="s" items="${staffList}" varStatus="loop">
                                    <tr>
                                        ...
                                        <td>
                                            <c:choose>
                                                <c:when test="${s.roleID == 3}"><span class="badge badge-reception">Lễ tân</span></c:when>
                                                <c:otherwise><span class="badge">Nhân viên phục vụ</span></c:otherwise>
                                            </c:choose>
                                        </td>
                                        <td>${s.email}</td>
                                        <td>${s.phoneNumber}</td>
