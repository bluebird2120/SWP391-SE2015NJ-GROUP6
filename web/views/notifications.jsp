...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .notif-page { max-width: 1080px; margin: 0 auto; padding: 24px 20px 60px; }
        ...
        .summary-info { display: flex; align-items: center; gap: 12px; }
        ...
        .item-msg-link { text-decoration: none; display: block; }
        .item-msg-link:hover .item-msg { color: #76493b; }
        .item-msg { color: #4a3528; font-size: 0.92rem; line-height: 1.5; margin: 0; transition: color 0.15s; }
        .item-time { color: #8a6e5a; font-size: 0.78rem; margin-top: 4px; }
        
        .btn { padding: 6px 12px; border-radius: 7px; border: none; cursor: pointer; font-size: 0.78rem; font-weight: 600; text-decoration: none; display: inline-flex; gap: 5px; align-items: center; transition: background 0.15s; }
        .btn-mark { background: #76493b; color: #fff; }
        .btn-mark:hover { background: #5d3a2e; }
        .btn-muted { background: #f4ebe4; color: #76493b; }
        .btn-muted:hover { background: #e8dbd0; }
                    ...
                    <c:forEach var="n" items="${notifications}" varStatus="status">
                        <div class="item ${n.isRead == 0 ? 'unread' : ''} ${status.index >= 5 ? 'previous-notif' : ''}"
                            ...
                            <div class="item-body">
                                ...
                                <c:choose>
                                    <c:when test="${isCustomer}">
                                        <a href="${pageContext.request.contextPath}/customer/notifications?action=readAndRedirect&notificationID=${n.notificationID}" class="item-msg-link">
